...
async function runJc(commandRaw: string, argsRaw: string[], workspace: string): Promise<string> {
  ...
  try {
    ...
    // caltools export) can NEVER hang the agent. 5 min covers the slowest real
...
async function main() {
  ...
  // This is the key thing Wideframe does that JCut was missing: the model starts
  // with real context — sources, sequences, memory — instead of having to discover
