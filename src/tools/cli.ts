      ...
      case "sequence-export-premiere": {
        ...
        if (!out && process.platform === "darwin") {
          try {
            ...
            // User cancelled or AppleScript failed -> fallback to default
          }
        }
        if (!out) {
          out = path.join(workspaceDir(ws), "renders", `${(seq.name || seq.id).replace(/[^\w.-]/g, "_")}.prproj`);
        }
        // Unless the user explicitly opts into the unverified built-in writer,
        // require the validated caltools engine — silently writing a possibly
        // -corrupt .prproj that opens EMPTY in Premiere would lose the user's work.
        const allowBuiltin = !!a["allow-unverified"];
        const res = await exportPremiere(ws, seq, out, allowBuiltin);
        emit({
          ok: true,
          output: res.output,
          engine: res.engine,
          ...
          clips: res.clips,
          captions_in_timeline: res.captions_in_timeline,
          transitions_in_timeline: res.transitions_in_timeline,
          note: res.engine === "caltools"
            ? "Exported via the Premiere engine (validated). Open in Premiere with File > Open."
            : "⚠ Exported with the BUILT-IN writer (Wideframe/caltools not detected). This format " +
              "is NOT validated against Premiere and may open as an empty timeline. Verify it opens " +
              "correctly before relying on it; for a guaranteed export, install Wideframe.",
        });
        break;
      }
